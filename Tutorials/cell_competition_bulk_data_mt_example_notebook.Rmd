...
MitoHEAR offers the possibility to perform a hierarchical clustering on the samples based on a distance matrix with the function *clustering_dist_ang*. Given a base, the distance between two samples is the angular distance of the allele frequencies. In this way we can represent the difference between two samples as a vector whose cordinates are the angular distances of the bases.
The total distance between two samples is the euclidean norm of the vector of difference between the two samples.
...
Finally we observe that the 98% of the top 50 bases defined in a unsupervised way (Wilcoxon test) are also bases that are different from mouse reference genome in the two cell lines. This reveals the high consistency of the analysis performed by MitoHEAR. 
```{r}
