...
load(paste0(your_path,"/MitoHEAR-master/inst/extdata/","after_qc.Rda"))
...
load(paste0(your_path,"/MitoHEAR-master/inst/extdata/","output_SNP_mt.Rda"))
...
#index_ci=seq(1,length(row.names(heteroplasmy_matrix_ci)))
```
...
plot_cells_coverage_epiblast(sum_matrix_qc,row.names(sum_matrix_qc),cluster_ci,interactive=FALSE)
plot_cells_coverage_epiblast(sum_matrix_qc,row.names(sum_matrix_qc),cluster_ci,interactive=TRUE)
...
```{r}
#plot_base_coverage(sum_matrix,sum_matrix_qc,row.names(sum_matrix),interactive=TRUE,text_size=10)

#plot_base_coverage(sum_matrix,sum_matrix_qc,row.names(sum_matrix_qc)[1:5],interactive=TRUE,text_size=10)
...
biomart_file=paste0(your_path,"/MitoHEAR-master/inst/extdata/","mt_start_end.txt")
...
fastaFile=paste0(your_path,"/MitoHEAR-master/inst/extdata/","Mus_musculus.GRCm38.dna.chromosome.MT.fa")
...
#bases_example=colnames(sum_matrix_qc)[1:10]
#plot_correlation_bases(bases_example,index_ci,heteroplasmy_matrix_ci)
```
...
#load("/Users/gabriele.lubatti/Desktop/Phd/Cell_Competition/Old_cell_competition/Script/dissimilarity_fmk_epi.Rda")

#library(destiny)
#dm=DiffusionMap(dissimilarity)
#DC1=dm$DC1
#DC2=dm$DC2
#coordinate_dm=data.frame(DC1,DC2)
#setwd("/Users/gabriele.lubatti/Desktop/Phd/Heteroplasmy_library_R/Input_data_R")
#save(coordinate_dm,file="coordinate_dm.Rda")


#setwd("/Users/gabriele.lubatti/Desktop/Phd/Heteroplasmy_library_R/Input_data_R")
#load(file="coordinate_dm.Rda")


load(paste0(your_path,"/MitoHEAR-master/inst/extdata/","coordinate_dm.Rda"))
...
cluster=after_qc[row.names(sum_matrix),]$cluster
#plot_coordinate_cluster(coordinate_dm ,cluster)
plot_coordinate_cluster_epiblast(coordinate_dm ,cluster)
...
```{r}
result_clustering_sc=clustering_dist_ang(heteroplasmy_matrix_ci,allele_matrix_ci,cluster_ci,length(row.names(heteroplasmy_matrix_ci)),deepSplit_param=0,minClusterSize_param=50,0.2,min_value=0.001,index=index_ci,relevant_bases = NULL)
...
p=plot_heteroplasmy_epiblast(top_dist[i],heteroplasmy_matrix_ci,cluster_ci,index_ci)
q=list(q,p)
}
q
