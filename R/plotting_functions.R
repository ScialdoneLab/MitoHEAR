...
plot_heteroplasmy=function (position, heteroplasmy_matrix, cluster, index)
{
  ...
  ggplot2::ggplot(data = dati_scatter_1, ggplot2::aes(x = factor(cluster), y = heteroplasmy,
  ...
  ggplot2::ggplot(data = data_plot_1, ggplot2::aes(x = batch, y = heteroplasmy,
    ...
    ggplot2::labs(y = "Heteroplasmy") + ggplot2::ggtitle(position) + ggplot2::scale_color_manual(values = c("#DD6400",
                                                                                                            "#0000FF", "#006400"), labels = c("Winner Epiblast",
                                                                                                                                              "Intermediate", "Loser Epiblast")) + ggplot2::theme(axis.text = ggplot2::element_text(size = text_size))
}
                     ...
                     text_size, index)
{
  ...
  ggplot2::ggplot(data = data_plot_1, ggplot2::aes(x = batch, y = heteroplasmy,
                                          col = factor(cluster))) + ggplot2::geom_boxplot(width = 0.9,
                                                                                          alpha = 0.8) + ggplot2::labs(col = "Cluster") + ggplot2::labs(x = "Batch") +
    ggplot2::labs(y = "Heteroplasmy") + ggplot2::ggtitle(position)+ ggplot2::theme(axis.text = ggplot2::element_text(size = text_size))
                       ...
                       name_y, name_title)
{
  ...
  add_test = paste("Wilcoxon test:", round(res$p.value, 4),
                   sep = " ")
  plot_gene = ggplot2::ggplot(data_plot, ggplot2::aes(x = label_all, y = distribution_all,
                                             col = label_all)) + ggplot2::geom_boxplot() + ggplot2::xlab(name_x) + ggplot2::ylab(name_y) +
    ggplot2::labs(col = name_x) + ggplot2::ggtitle(paste(name_title, add_test,
                                                sep = " - "))
  return(plot_gene)
}
...
plot_heteroplasmy_epiblast=function (position, heteroplasmy_matrix, cluster, index)
{
  ...
  dati_scatter_1 = data.frame(as.character(cluster), heteroplasmy)
  ggplot2::ggplot(data = dati_scatter_1, ggplot2::aes(x = factor(cluster, levels = c("Winner Epiblast",
                                         ...
                                         names_allele_qc, names_position_qc, size_text, index)
{
  ...
  plot_1 = ggplot2::ggplot(data = data_plot_1, ggplot2::aes(x = factor(cluster,
    ...
    ggplot2::geom_point() + ggplot2::labs(col = "Cluster") + ggplot2::labs(y = "Allele Frequency") + ggplot2::labs(x = "Cluster") + ggplot2::labs(y = "Allele frequency") +
    ggplot2::ggtitle(allele[1]) + ggplot2::ylim(0, 1) + ggplot2::scale_color_manual(values = c("#DD6400",
                                                                                      "#0000FF", "#006400"), labels = c("Winner Epiblast",
                                                                                                                        "Intermediate", "Loser Epiblast")) + ggplot2::theme(axis.text = ggplot2::element_text(size = size_text))



  plot_2 = ggplot2::ggplot(data = data_plot_2, ggplot2::aes(x = factor(cluster,
                                                              levels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"),
                                                              ordered = TRUE), y = allele_2, col = factor(cluster,
                                                                                                          levels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"),
                                                                                                          ordered = TRUE))) + ggplot2::geom_boxplot(width = 0.9, alpha = 0.8) +
    ggplot2::geom_point() + ggplot2::labs(col = "Cluster") + ggplot2::labs(x = "Cluster") + ggplot2::labs(y = "Allele Frequency")+ ggplot2::ggtitle(allele[2]) + ggplot2::ylim(0, 1) + ggplot2::scale_color_manual(values = c("#DD6400",
                                                                                                                                                                                                                     "#0000FF", "#006400"), labels = c("Winner Epiblast",
                                                                                                                                                                                                                                                       "Intermediate", "Loser Epiblast")) + ggplot2::theme(axis.text = ggplot2::element_text(size = size_text))
  plot_3 = ggplot2::ggplot(data = data_plot_3, ggplot2::aes(x = factor(cluster,
                                                              levels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"),
...
plot_cells_coverage_epiblast=function(sum_matrix,cells_selected,cluster,interactive=FALSE){
  ...
  if (interactive==FALSE){
    p<-ggplot2::ggplot(data=df, ggplot2::aes(x=cell_id_plot, y=df[,2],fill=cluster_plot_plot)) +
      ...
      ggplot2::theme_minimal()+ggplot2::ylab("MT coverage per thousand reads")+ggplot2::xlab("Cells")+ggplot2::ggtitle("MT coverage per cell")+ggplot2::scale_fill_manual(values = c("#DD6400", "#0000FF", "#006400"),
                                                                                                                             labels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"))+ggplot2::labs(fill="Category")+ggplot2::theme(
                                                                                                                               axis.text.x=ggplot2::element_blank(),
                                                                                                                               axis.ticks.x=ggplot2::element_blank())+ggplot2::xlab("Cells")
    return(list(p))
  }
  if (interactive==TRUE){
...
plot_cells_coverage=function(sum_matrix,cells_selected,cluster,interactive=FALSE){
  ...
  if (interactive==FALSE){
    p<-ggplot2::ggplot(data=df, ggplot2::aes(x=cell_id_plot, y=df[,2],fill=df[,3])) +
      ggplot2::geom_bar(stat="identity",)+
      ggplot2::theme_minimal()+ggplot2::ylab("MT coverage per thousand reads")+ggplot2::xlab("Cells")+ggplot2::ggtitle("MT coverage per cell")+ggplot2::labs(fill="Category")+ggplot2::theme(
        axis.text.x=ggplot2::element_blank(),
        axis.ticks.x=ggplot2::element_blank())+ggplot2::xlab("Cells")
    return(list(p))
  }
  if (interactive==TRUE){
    ...
    for (i in 1:type){
      color_plot[df[,3]==levels_type[i]]=MitoHEAR:::gg_color_hue(type)[i]
    }
    fig <- plotly::plot_ly(
      x = (cell_id_plot),
      y = df[,2],
...
plot_base_coverage=function(sum_matrix,sum_matrix_qc,selected_cells,interactive = FALSE,text_size=10){
  ...
  if (interactive==FALSE){
    ...
    p<-ggplot2::ggplot(data=df, ggplot2::aes(x=(df[,1]), y=df[,2],fill=df[,3])) +
      ggplot2::geom_bar(stat="identity",)+
      ggplot2::theme_minimal()+ggplot2::ylab("Mean reads per base")+ggplot2::ggtitle("MT coverage per base")+ggplot2::labs(fill="Category")+ggplot2::xlab("MT bases")+ggplot2::scale_x_continuous(breaks = round(seq(min((df[,1])), max((df[,1])), by = 1000),1))+ggplot2::theme(text = ggplot2::element_text(size = text_size),
...
plot_spider_chart=function(name_base,cluster,heteroplasmy_matrix,index){
  ...
  build_radarchart(student1_data,color=MitoHEAR:::gg_color_hue(1)[1], caxislabels = c(0, 5, 10, 15, 20),title = name_base)
  par(op)
...
plot_coordinate_heteroplasmy=function(coordinate_dm,heteroplasmy_matrix,index,name_base){
  if (is.null(index)){
    ...
    ggplot2::ggplot(dat, ggplot2::aes(coordinate_dm[,1], coordinate_dm[,1])) +
      ...
      ggplot2::scale_colour_gradient(low = "white", high = "midnight blue")+ggplot2::labs(col="Heteroplasmy")+ggplot2::ggtitle(paste0("Heteroplasmy level of ",name_base))
...
plot_coordinate_cluster_epiblast=function(coordinate_dm,cluster){
  ...
  diffu<-ggplot2::ggplot(dat, ggplot2::aes(x=coordinate_dm[,1] , y=coordinate_dm[,2] , color= Cluster)) + ggplot2::geom_point()


  diffu + ggplot2::scale_color_manual(values=c("#DD6400",  "#0000FF","#006400"),labels=c("Normal (winner) Epiblast","Intermediate", "Loser Epiblast"))+ggplot2::labs(x = "DC1",y="DC2") + ggplot2::ggtitle("Diffusion Map")}
...
plot_coordinate_cluster=function(coordinate_dm,cluster){
  ...
  diffu +ggplot2::labs(x = "DC1",y="DC2") + ggplot2::ggtitle("Diffusion Map")}
...
plot_heteroplasmy_variability=function(heteroplasmy_matrix,cluster,threshold=0.1,frac=FALSE,index){
  ...
  if(frac==TRUE){
    ...
    df <- data.frame(stage=stages,
                     number=number)
    p=ggplot2::ggplot(data=df, ggplot2::aes(x=stages, y=number)) +
      ggplot2::geom_bar(stat="identity", color="blue", fill="blue")+ggplot2::ggtitle(paste("Fraction of cells with H>",threshold," across stages",sep=""))+ggplot2::ylab("Fraction of cells")+ggplot2::xlab("Stage")
